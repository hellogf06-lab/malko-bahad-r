        ...
        if (!data.user) {
          alert('Oturum yok! Lütfen tekrar giriş yapın.');
            ...
            const handleExpenseSubmit = (data) => {
              // Formdan gelen veriyi Supabase'ın beklediği yapıya dönüştür
              const giderData = {
                aciklama: data.title,
                tutar: String(Number(data.amount)),
                kategori: data.category,
                tarih: data.date ? (typeof data.date === 'string' ? data.date : data.date.toISOString().slice(0, 10)) : '',
                notes: null
              };
              const onError = (error) => {
